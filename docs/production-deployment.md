...
7. Export `FLOWTALLY_BOOTSTRAP_ADMIN_URL` privately as the `avnadmin` connection to `flowtally_prod`, then run `python scripts/bootstrap_postgres.py --phase provision`. On Aiven, the command claims provider-owned `public` schema through `aiven_extras.claim_public_schema_ownership()` when needed; `avnadmin` may remain the schema owner. It applies the production role boundary and grants without creating roles or changing passwords.
...
On Aiven, provider-owned `public` schema is claimed by `avnadmin` through `aiven_extras.claim_public_schema_ownership()`. The migrator does not need to own `public`; it receives `CREATE` and `USAGE`, and migration-created tables, sequences, and functions are owned by the migrator connection. The runtime role receives only application privileges and must not own the database, schema, or protected tables. RLS and FORCE RLS remain the database boundary; role membership must not provide a bypass path. `scripts/bootstrap_postgres.py --phase provision` applies the role flags, schema/database grants, and Aiven claim, while `--phase finalize` reapplies current-object grants and applies default privileges through the migrator connection. If Aiven refuses the claim, the command fails with an actionable message rather than attempting an unsafe schema transfer.
