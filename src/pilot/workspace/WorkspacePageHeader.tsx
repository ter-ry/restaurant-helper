...
export function WorkspacePageHeader({ eyebrow, title, description, actions, metrics, className = "" }: WorkspacePageHeaderProps) {
  return (
    <section className={`surface-panel rounded-2xl border border-line bg-white p-5 shadow-soft sm:p-6 ${className}`}>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {metrics.map((metric) => (
            <div key={metric.label} className="rounded-2xl border border-line bg-slate-50 p-4">
              ...
              <p className="mt-2 min-w-0 text-2xl font-bold text-ink">{metric.value}</p>
