...
export function WorkspacePageHeader({ eyebrow, title, description, actions, metrics, className = "" }: WorkspacePageHeaderProps) {
  return (
    <section className={`pilot-page-header surface-panel rounded-2xl border border-line bg-white p-3 shadow-soft sm:p-4 ${className}`}>
      ...
      {metrics?.length ? (
        <div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
          {metrics.map((metric) => (
            <div key={metric.label} className="pilot-metric-card rounded-xl border border-line bg-slate-50 p-3">
