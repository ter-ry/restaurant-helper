                ...
                <section className="order-2 space-y-4" data-testid="purchase-details-panel">
                  <div className="grid gap-3 md:grid-cols-3">
                    <label className="block rounded-2xl border border-line bg-slate-50 p-4">
                      ...
                      <input className="input mt-1 w-32" type="number" step="1" value={draft.subtotal} onFocus={selectZeroValue} onChange={(event) => setDraft((current) => ({ ...current, subtotal: Number(event.target.value), totalAmount: Number(event.target.value) + Number(current.tax || 0) }))} disabled={finalizedStatus} />
                    </label>
                    <label className="block rounded-2xl border border-line bg-slate-50 p-4">
                      <span className="text-sm font-semibold text-ink">Tax</span>
                      <input className="input mt-1 w-32" type="number" step="1" value={draft.tax} onFocus={selectZeroValue} onChange={(event) => setTax(Number(event.target.value))} disabled={finalizedStatus} />
                    </label>
                    <label className="block rounded-2xl border border-line bg-slate-50 p-4">
                      <span className="text-sm font-semibold text-ink">Total</span>
                      <input className="input mt-1 w-32 font-semibold" type="number" step="1" value={draft.totalAmount} onFocus={selectZeroValue} onChange={(event) => setDraft((current) => ({ ...current, totalAmount: Number(event.target.value) }))} disabled={finalizedStatus} />
                    </label>
                  </div>
                        ...
                        <div key={line.id ?? index} className="rounded-2xl border border-line bg-white p-4">
                          ...
                          <div className="mt-3 grid gap-3 md:grid-cols-[minmax(7rem,0.85fr)_minmax(8rem,1fr)_minmax(8rem,1fr)_minmax(8rem,1fr)]">
                            <label className="block">
                              ...
                              <input className="input mt-1 w-28" type="number" step="1" value={line.quantity} onFocus={selectZeroValue} onChange={(event) => updateLine(index, (current) => ({ ...current, quantity: Number(event.target.value), lineTotal: Number(event.target.value) * Number(current.unitPrice || 0) }))} disabled={finalizedStatus} />
                            </label>
                            <label className="block">
                            ...
                            <label className="block">
                              ...
                              <input className="input mt-1 w-32" type="number" step="1" value={line.unitPrice} onFocus={selectZeroValue} onChange={(event) => updateLine(index, (current) => ({ ...current, unitPrice: Number(event.target.value), lineTotal: Number(current.quantity || 0) * Number(event.target.value) }))} disabled={finalizedStatus} />
                            </label>
                            <label className="block">
                              <span className="text-xs font-bold uppercase tracking-wide text-muted">Line total</span>
                              <input className="input mt-1 w-32 font-semibold" type="number" step="1" value={line.lineTotal} onFocus={selectZeroValue} onChange={(event) => updateLine(index, (current) => ({ ...current, lineTotal: Number(event.target.value) }))} disabled={finalizedStatus} />
                            </label>
                          </div>
