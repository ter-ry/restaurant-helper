...
export function Modal({ title, children, onClose, labelledBy, size = "large" }: { title: string; children: ReactNode; onClose: () => void; labelledBy?: string; size?: "small" | "large" | "full" | "fullscreen" }) {
  ...
  return <div className={`pilot-modal-backdrop fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 ${size === "fullscreen" ? "sm:items-stretch" : "sm:items-center sm:p-2"}`} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby={headingId} className={`pilot-modal w-full ${width} ${shell} overflow-y-auto bg-white shadow-2xl`}>
      <header className="pilot-modal-header sticky top-0 z-10 flex items-center justify-between border-b border-line bg-white px-4 py-3"><h2 id={headingId} className="text-lg font-bold text-ink">{title}</h2><button ref={closeRef} type="button" aria-label={`Close ${title}`} onClick={onClose} className="min-h-10 rounded-xl px-3 text-sm font-semibold text-muted hover:bg-slate-100 hover:text-ink">Close</button></header>
