...
def test_demo_mode_blocks_api_writes_but_keeps_reads_and_session_mechanics(app, client, monkeypatch):
    ...
    assert client.get("/api/auth/google/callback?code=demo&state=demo").status_code == 403
...
def test_demo_login_is_the_only_public_auth_entry(app, client):
    app.config["FLOWTALLY_DEMO_READ_ONLY"] = True
    csrf = client.get("/api/auth/csrf").get_json()["csrfToken"]
    response = client.post("/api/auth/demo-login", headers={"X-CSRFToken": csrf})
    assert response.status_code == 200
    assert response.get_json()["user"]["email"] == "owner@flowtally.local"
