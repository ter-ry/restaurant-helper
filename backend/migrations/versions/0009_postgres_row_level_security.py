...
def _apply_sql(execute_sql: Callable[[str], None]) -> None:
    ...
    def create_policy(table: str, policy_name: str, using_sql: str, with_check_sql: str | None = None) -> None:
    ...
    for table in [
        ...
        create_policy(table, f"flowtally_{table}_tenant_access", "flowtally_has_org_access(organization_id)")
...
def remove_postgres_rls(connection: Connection) -> None:
    ...
    for table, policy_name in [
        ...
        execute_sql(f'DROP POLICY IF EXISTS "{policy_name}" ON "{table}"')
        execute_sql(f'ALTER TABLE "{table}" NO FORCE ROW LEVEL SECURITY')
        execute_sql(f'ALTER TABLE "{table}" DISABLE ROW LEVEL SECURITY')
    ...
    for func in [
        ...
        execute_sql(f"DROP FUNCTION IF EXISTS {func} CASCADE")
